  ...
  public:
    ...
    */
    ...
    /*  array_[index] = pair{key, val}
        顺次移动
    */
    void ModKeyAt(const KeyType &key , int index) { 
    ...
    */
    ...
    auto ToString() const -> std::string {
      ...
      for (int i = 0; i < GetSize(); i++) {
        ...
        kstr.append("( " + key.first.to_string() + " , " +
...
#define MaxSize 40000
    ...
    void init(std::string file_name, int* header_id) { 
      ...
      if(!root.good()) {
        root.clear();
        root.open("root", std::ios::binary | std::ios::trunc | std::ios::out | std::ios::in);
        *header_id = -1;
      } else {
    ...
    void del_content(int pos_, int tp = 0) {
      ...
      //             << tmp[pos_].head->pin << " " << to_string(tmp[pos_].content) << tmp[pos_].head->count
      //                                                      << std::endl;
...
public:
  ...
  void printtree(Ptr rt, int dep = 0) {
    for(int i = 0; i < dep ; i++) std::cerr << "  ";
    std::cerr << to_string(rt.content) << std::endl;
    if(rt.content->IsLeafPage()) return;
    for(int i = 0; i < rt.content->GetSize(); ++i)
      printtree(Ptr(rt.bpm, nullptr, static_cast<BPlusTreeInternalPage *>(rt.content)->ValueAt(i)), dep+1);
  }
  ...
  void FindPosVector(const KeyType &key, sjtu::vector<Ptr> *v,
    ...
    v->push_back(rt);
    // std::cerr << "route for " << key.second << " " << rt.content << " " << to_string(rt.content) << " |";
    while (rt.content && !rt->IsLeafPage()) {
      auto res = (static_cast<BPlusTreeInternalPage *>(rt.content)->find(key));
  ...
  void update(Ptr &fa, BPlusTreePage *fa_cont, Ptr &pre,
    ...
    if (dx > 0) {
      ...
      // pos_cont = bro + 1
      auto res = MappingType{KeyAt(pre_cont, pre_cont->GetSize() - 1), ValAt(pre_cont, pre_cont->GetSize() - 1)};
      SetKeyAt(nxt_cont, res.first, res.second, 0);
      DelAt(pre_cont, pre_cont->GetSize() - 1);
    }
    static_cast<BPlusTreeInternalPage*>(fa_cont)->ModKeyAt( KeyAt(nxt_cont, 0), index + 1);
  ...
  void merge(Ptr &fa, BPlusTreePage *fa_cont, Ptr &pre, BPlusTreePage *pre_cont,
    ...
    if(pre_cont->IsLeafPage())
      static_cast<BPlusTreeLeafPage*>(pre_cont)->SetNextPageId(static_cast<BPlusTreeLeafPage*>(nxt_cont)->GetNextPageId());
    //printtree(Ptr(&bpm, nullptr, header_page_id_));
    pre.Dirty(), nxt.Dirty();
    nxt_cont->SetSize(0);
  }
  ...
  void Remove(const KeyType &key) {
    ...
    DelAt(cont, index);
    pos.Dirty();
    if(cont->GetSize() == 1 && !pos.content->IsLeafPage() && pos.head->pos == header_page_id_) {
...
#define rf(i,a,b) for(int i = a;i >= b; --i)
  ...
  rf(i,range, 0) {
    ...
    std::cout << a[i] << " num = " << result.size() << std::endl;
    //Bpt.printtree();
  }
  // Bpt.Insert({s + s,2},2);
}
